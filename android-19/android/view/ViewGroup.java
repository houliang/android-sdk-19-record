...
public abstract class ViewGroup extends View implements ViewParent, ViewManager {
     ...
     */
    /*    
	View类中的findViewById方法，所有的控件都继承了View，也就有了这个方法，看看这个方法
	其实很简单，就是直接调用了findViewTraversal函数:
	...
	单个View没有什么好find的，直接进行比较，如果find给定的ID和当前View的ID相同，则返回当前
	View的引用，否则返回null
    protected View findViewTraversal(int id) {
 	...
 	然后我就纳闷了，在ViewGroup类里面竟然都没有findViewById这个方法，后来想想也是，已经继承了View的,
 	就不需要再自己定义一个了，于是可以想到ViewGroup类应该是覆盖了findViewById调用的濉秄indViewTraversal
 	方法，果然就在下面找到了，这个方法也很简单了:
 	(1)比较自身
 	(2)比较ViewGroup包含的每一个View，直到找到为止，而且在比较View的时候，调用了View的findViewById
    */
    @Override
    protected View findViewTraversal(int id) {
        if (id == mID) {
