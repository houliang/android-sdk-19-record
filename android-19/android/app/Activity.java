...
public class Activity extends ContextThemeWrapper
    ...
    public Object getSystemService(String name) {
        ...
        if (WINDOW_SERVICE.equals(name)) {//"window"
            return mWindowManager;
        } else if (SEARCH_SERVICE.equals(name)) { //"search"
            ensureSearchManager();
    ...
    final void attach(Context context, ActivityThread aThread,
		...
		//这里mWindow实际上是PhoneWindow(extends Window),但是PhoneWindow没有覆盖setWindowManager方法，所以实际上调用的是
		//Window的setWindowManager方法，实际上就是创建了一个WindowManagerImpl对象，这里面要注意context的实际类型是ContextImpl，所以在调用
		//getSystemService函数的时候实际上是调用的ContextImpl.getSystemService函数，这里传递的WINDOW_SERVICE字符串，会在ContextImpl的
		//static块中创建一个WindowManagerImpl对象，下次看的时候直接在ContextImpl.java里面搜索WindowManagerImpl就能看到
