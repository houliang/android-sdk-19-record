...
public final class ActivityThread {
    ...
    private class ApplicationThread extends ApplicationThreadNative {
        ...
        public final void scheduleLaunchActivity(Intent intent, IBinder token, int ident,
			...
			/*
			在MessageQueue中增加LAUNCH_ACTIVITY消息，Hander H处理的时候增加了ActivityClientRecord
			中packageinfo的值，然后直接调用了handleLaunchActivity函数，中间没有经过中转
			*/
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case LAUNCH_ACTIVITY: {
					/*scheduleLaunchActivity会在最后send一条LAUNCH_ACTIVITY Message
					  而且整个文件中只有这一个地方send LAUNCH_ACTIVITY消息
					*/
                    Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "activityStart");
					
					/*send LAUNCH_ACTIVITY Message的时候，会创建一个新的record，里面包含了
					  Activity的启动参数
					*/
	...
	//这个函数在handleMessage的时候调用了，目的是创建一个LoadedApk对象，并放到 里面
	//调用getPackageInfo函数去创建对象
...
/*
第一次为Activity创建LoadedApk对象:
创建新的LoadedApk对象，includeCode参数为true,所以创建完的LoadedApk会存放在mPackages里面，下次
要用到这个对象的时候，直接mPackages里根据包名称来找，当然第一次调用的时候会new一个对象出来，
注意这里面是根据包名来找的，说明这个main ui thread里面对于一个包名只会在mPackages存储一个LoadedApk对象
*/
    private LoadedApk getPackageInfo(ApplicationInfo aInfo, CompatibilityInfo compatInfo,
            ClassLoader baseLoader, boolean securityViolation, boolean includeCode) {
        synchronized (mResourcesManager) {
			//先看根据包名在mPackages或者mResourcePackages里面找
            WeakReference<LoadedApk> ref;
            if (includeCode) {
			...
			//找不到 packageInfo 就是null,这时候就会new一个
            if (packageInfo == null || (packageInfo.mResources != null
                    && !packageInfo.mResources.getAssets().isUpToDate())) {
	...
	/*
	这个函数是相当的重要，首先由
	(1)scheduleLaunchActivity(sendMessage(LAUNCH_ACTIVITY))
	(2)handleLaunchActivity 
	(3)performLaunchActivity  创建实际与Activity相关的对象，包括Activity本身
		这个函数的第一个参数ActivityClientRecord，在调用这个函数之前里面的Activity成员是空的，这个函数
	创建了Activity对象，然后还调用了Activity的onCreate函数
	...
	*/
    private Activity performLaunchActivity(ActivityClientRecord r, Intent customIntent) {
        ...
        try {
			//创建Activity对象实体,利用载入class对象，然后newInstance的方法创建对象
			//但是Activity没有任何构造函数，因此创建对象的时候，没有做任何的处理，那么
			//Activity的状态没有发生任何变化
        ...
        try {
            ...
            if (activity != null) {
				...
				//这里面把前面创建的Application ContextImpl等等的参数和上面创建的Activity绑定在一起
				//点进去一看原来这里面还做了别的事情，创建了WindowManager的实体WindowManagerImpl
				//是在ContextImpl的getSystemService里面创建的,另外还创建了PhoneWindow
				...
				//这里面会去调用Activity的onCreate函数，这个很重要，因为需要去理解这个调用的时机，这时候才创建了
				//activity对象、相关的Application、ContextImpl
				...
				//------------给形参ActivityClientRecord的activity赋值，现在Activity算是有了实体-----------------
                r.activity = activity;
	...
	/*
	scheduleLaunchActivity为创建一个新的Activity做准备
