...
/*
Application和ContextImpl都是直接继承了Context

创建Application对象，这里面是现创建了一个ContextImpl，最后将ContextImpl对象的引用放在了Application对象里面，
搞得还有点复杂，一时间反应不过来，先这样吧
...
*/
    public Application makeApplication(boolean forceDefaultAppClass,
		...
		//如果没有创建Application，系统会自己给定义一个，类名称就是"android.app.Application"
        ...
        try {
		//这里面就热闹了，里面创建了一个ContextImpl,这个类可真是难找，搞不清楚context,这是个abstract class但是却用的很多，
		//这里找到了，context的实现类应该是这里的ContextImpl，在这里new的时候基本上什么也没有做，但是后面有调用了
		//ContextImpl.init函数，对ContextImpl进行初始化
		
		//Application和ContextImpl都是直接继承了Context，下面用上面创建的ContextImpl创建了一个Application对象
